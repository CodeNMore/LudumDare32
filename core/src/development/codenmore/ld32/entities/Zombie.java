...
public class Zombie extends Enemy {
	...
	private static TextureRegion zombieUp = Assets.getRegion("enemyup"),
			zombieDown = Assets.getRegion("enemydown"), zombieLeft = Assets
					.getRegion("enemyleft"), zombieRight = Assets
					.getRegion("enemyright");

	private float findWaitTimer = 0;
	private boolean lazerZombie;

	public Zombie(Level level, float x, float y, boolean lazerZombie,
			boolean superZomb) {
	...
	public void tick(float delta) {
		...
		if (level.getEntityManager().getPlayer().getBounds()
				.overlaps(getBounds())) {
			onTouchHurt();
		}
	}
